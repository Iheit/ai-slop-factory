...
def cube(name, loc, scale, material=WOOD, bevel=0.05, col=None):
    ...
    if col and o.name not in col.objects: col.objects.link(o); bpy.context.collection.objects.unlink(o)
    return o
...
def cyl(name, loc, r, depth, material=METAL, verts=20, col=None):
    ...
    if col: col.objects.link(o); bpy.context.collection.objects.unlink(o)
    return o

def sphere(name, loc, r, material=METAL, col=None):
    bpy.ops.mesh.primitive_uv_sphere_add(segments=20, ring_count=10, radius=r, location=loc); o=bpy.context.object; o.name=name; o.data.materials.append(material)
    if col: col.objects.link(o); bpy.context.collection.objects.unlink(o)
    return o
...
def plaque(name, text='WARNING', col=None):
    ...
    bpy.ops.object.text_add(location=(0,-0.035,0),rotation=(math.pi/2,0,0)); t=bpy.context.object; t.data.body=text; t.data.align_x='CENTER'; t.data.size=.12; t.data.extrude=.008; t.data.materials.append(PAPER); t.name=name+'_Text'
    if col: col.objects.link(t); bpy.context.collection.objects.unlink(t)
    return base
...
def build(kind,name,col):
    ...
    if kind=='wall': o=[cube(name,(0,0,1.5),(4,.18,3),WOOD,.04,col)]
        ...
        o=[cube(name,(0,0,1.2),(1.0,.14,2.4),WOOD,.035,col),cube(name+'_Handle',(.35,-.10,1.2),(.08,.08,.08),METAL,.02,col)]
    elif kind=='window':
        ...
        o=[cube(name,(0,0,.48),(2.0,.8,.35),FABRIC,.12,col),cube(name+'_Back',(0,.27,1.0),(2.0,.28,1.0),FABRIC,.12,col)]
        for x in (-.78,.78): o.append(cube(name+'_Arm',(x,0,0.8),(.25,.8,.7),FABRIC,.1,col))
    elif kind=='bed':
        o=[cube(name,(0,0,.45),(2.0,2.0,.28),DARKWOOD,.04,col),cube(name+'_Mattress',(0,0,.7),(1.9,1.9,.35),FABRIC,.1,col),cube(name+'_Headboard',(0,.86,1.45),(2.0,.12,1.5),WOOD,.04,col)]
    elif kind=='lamp':
        o=[cyl(name,(0,0,.65),.06,1.2,METAL,16,col),cyl(name+'_Base',(0,0,.05),.22,.1,METAL,20,col),cyl(name+'_Shade',(0,0,1.35),.28,.42,FABRIC,24,col)]
    elif kind=='rug': o=[cube(name,(0,0,.025),(2.5,1.6,.04),FABRIC,.04,col)]
    elif kind=='mirror':
        o=[cube(name,(0,0,1.2),(1.0,.08,1.5),DARKWOOD,.04,col),cube(name+'_Glass',(0,-.05,1.2),(.82,.025,1.3),GLASS,.01,col)]
    elif kind=='toilet':
        o=[cyl(name,(0,0,.38),.34,.5,CERAMIC,24,col),cyl(name+'_Tank',(0,.2,.85),.3,.7,CERAMIC,20,col),cyl(name+'_Seat',(0,0,.65),.29,.08,CERAMIC,24,col)]
    ...
    elif kind=='barrel': o=[cyl(name,(0,0,.65),.45,1.3,WOOD,24,col),cyl(name+'_Ring',(0,0,.35),.47,.06,METAL,24,col),cyl(name+'_Ring',(0,0,.95),.47,.06,METAL,24,col)]
        ...
        bpy.ops.mesh.primitive_cylinder_add(vertices=32,radius=1.0,depth=.04,location=(0,0,.03)); x=bpy.context.object; x.name=name; x.data.materials.append(FABRIC); col.objects.link(x); bpy.context.collection.objects.unlink(x); o=[x]
    elif kind=='stair':
        for i in range(6): o.append(cube(name+f'_Step{i}',(i*.35,0,i*.22),(0.5,1.2,.22),WOOD,.025,col))
    else: o=[cube(name,(0,0,.5),(1,1,1),WOOD,.03,col)]
...
def generate_batch(batch_name, specs):
    ...
    for name,kind in specs:
        build(kind,name,col)
    bpy.ops.object.select_all(action='DESELECT')
    print(f'{batch_name}: generated {len(specs)} assets')
